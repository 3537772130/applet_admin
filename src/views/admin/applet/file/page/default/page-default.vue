...
<template>
  <div>
    <el-main v-loading="loading" element-loading-text="加载中" class="default-main">
      <!--餐饮-->
      <el-tabs v-model="showTabs" type="border-card" v-if="fileTypeId == 1">
      ...
      <!--百货超市/便利店-->
      <el-tabs v-model="showTabs" type="border-card" v-if="fileTypeId == 2">
        <el-tab-pane v-for="(item, index) in pageList"
                     :key="index"
                     :label="item.pageName" :name="'page' + item.id">
      ...
      <!--水果专卖-->
      <el-tabs v-model="showTabs" type="border-card" v-if="fileTypeId == 3">
        <el-tab-pane v-for="(item, index) in pageList" :key="index"
                     :label="item.pageName" :name="'page' + item.id">
          {{item.pageLogo + '-' + item.pageName}}
        </el-tab-pane>
